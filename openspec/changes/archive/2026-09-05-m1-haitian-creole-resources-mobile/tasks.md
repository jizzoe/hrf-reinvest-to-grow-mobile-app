...
- [x] 2.1 Move the 68 existing keys verbatim into `src/i18n/en.json` and
...
- [x] 2.2 Add `src/i18n/ht.json` with machine-generated Haitian Creole values
  and a `_meta` entry recording review status, generation method, and date.
- [x] 2.3 Add a key-parity test asserting each language file carries exactly
  the English key set, with no missing or extra keys.

## 3. Haitian Creole Support

- [x] 3.1 Extend the language type, device-language resolution, the existing
  language selector, and the text-to-speech language mapping to Haitian Creole.
- [x] 3.2 Map a Haitian Creole interface to `fr-HT` number and date formatting,
